...
def _generate_fins(c, x, fin_size, bend):
    ...
    for i in range(num_fins):
        y = y0 + i * 2 * fin_size[1]
        rectangle = c << gf.pcells.rectangle(
...
def _generate_bends(c, x_top, x_bot, dx, dy, gap):
    input_bend_top = (
        c << gf.pcells.bend_s(size=(dx, dy), cross_section=x_top.copy()).mirror()
    )

    input_bend_bottom = c << gf.pcells.bend_s(
...
def _generate_straights(c, length, x_top, x_bot, input_bend_top, input_bend_bottom):
    top_straight = c << gf.pcells.straight(length=length, cross_section=x_top)

    bottom_straight = c << gf.pcells.straight(length=length, cross_section=x_bot)
