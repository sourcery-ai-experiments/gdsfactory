...
from gdsfactory.pcells.crossing_waveguide import compensation_path, crossing45
from gdsfactory.component import get_ports_facing
...
def component_sequence_to_str(sequence):
    """Transform a sequence of pcells (such as the one obtained from.
...
def component_lattice(
    ...
    pcells with the same x spacing between input/output ports Lines must
    have pcells with the same y spacing between input/output ports.
    ...
    .. plot::
      ...
      import gdsfactory as gf
      from gdsfactory.pcells.crossing_waveguide import crossing45
      from gdsfactory.pcells.crossing_waveguide import compensation_path

      symbol_to_component =  {
            "C": gf.routing.fanout2x2(component=gf.pcells.coupler(), port_spacing=40.0),
            "X": crossing45(port_spacing=40.0),
            "-": compensation_path(crossing45=crossing45(port_spacing=40.0)),
      }
      c = gf.pcells.component_lattice(symbol_to_component=symbol_to_component)
      c.plot()
    """
    ...
    # Find y spacing and check that all pcells have same y spacing
    y_spacing = None
    ...
    for i in keys:
        ...
        for c in col:
            ...
            if c in symbol_to_component.keys():
                ...
                nb_inputs = pcells_to_nb_input_ports[c]
                skip = nb_inputs - 1
...
if __name__ == "__main__":
    ...
    c = gf.pcells.component_lattice(symbol_to_component=pcells_dict)
    # c= gf.routing.fanout2x2(component=gf.pcells.coupler(), port_spacing=40.0)
