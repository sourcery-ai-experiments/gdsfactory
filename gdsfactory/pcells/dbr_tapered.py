...
def _generate_fins(
    ...
    for i in range(int(num_fins)):
        y = y0 + i * 2 * fin_size[1]
        rectangle_input = c << gf.pcells.rectangle(
...
def dbr_tapered(
    ...
    **kwargs,
    ...
    straight = c << gf.pcells.straight(length=length, cross_section=xs.copy(width=w1))

    output_taper = c << gf.pcells.taper(
        length=taper_length,
        width1=w1,
        width2=xs.width,
        cross_section=xs.copy(width=xs.width),
    ...
    periodic_structures = c << gf.pcells.array(
        gf.pcells.rectangle((period * dc, w2)), (period, 0), num
    )
