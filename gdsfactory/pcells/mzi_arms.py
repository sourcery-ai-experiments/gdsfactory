...
def mzi_arms(
    ...
    **kwargs,
    ...
    You can find a more robust mzi in gf.pcells.mzi
    ...
    if with_splitter:
        c.add_ports(cin.get_ports_list(angle=180), prefix="in")
    else:
        c.add_port("o1", port=bot_arm.ports["o1"])
        c.add_port("o2", port=top_arm.ports["o1"])

    c.add_ports(cout.get_ports_list(angle=0), prefix="out")
...
if __name__ == "__main__":
    ...
    # c.pprint_netlist()
    mmi2x2 = gf.partial(gf.pcells.mmi2x2, width_mmi=5, gap_mmi=2)
    c = mzi_arms(delta_length=10, combiner=mmi2x2)
    c.show(show_ports=True)

    def bend_s(length: float = 10, **kwargs):
        return gf.pcells.bend_s(size=(length, 10), **kwargs)

    # c = mzi_arms(
    #     delta_length=50,
    #     # straight_x_top=bend_s,
    #     # straight_x_bot=gf.compose(gf.functions.mirror, bend_s),
    #     # straight_x_top=gf.pcells.straight_heater_meander,
    #     # straight_x_bot=gf.pcells.straight_heater_meander,
    #     # straight_x_top=gf.pcells.straight_heater_metal,
    #     # straight_x_bot=gf.pcells.straight_heater_metal,
    #     # length_x=300,
    #     # delta_yright=-20,
    #     # length_x_bot=300,
    #     # length_y=1.8,
