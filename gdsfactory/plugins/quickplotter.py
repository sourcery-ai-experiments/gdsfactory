...
from gdsfactory.component_reference import Instance
...
def quickplot(items, **kwargs):  # noqa: C901
    ...
    # Iterate through each Component/Instance/Polygon
    if not isinstance(items, list):
        items = [items]
    LAYER_VIEWS = get_layer_views()
    all_lv_tuples = LAYER_VIEWS.get_layer_tuples()
    for item in items:
        if isinstance(item, (Component, Instance)):
            ...
            # If item is a Component or Instance, draw ports
            if isinstance(item, (Component, Instance)) and show_ports is True:
...
def quickplot2(item_list, *args, **kwargs):
    ...
    for element in item_list:
        if isinstance(
            ...
            # If element is a Instance, draw ports as subports
