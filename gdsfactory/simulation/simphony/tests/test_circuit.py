...
def test_circuit_transmission(data_regression, check: bool = True):
    component = gf.pcells.mzi(delta_length=10)
    circuit = component_to_circuit(component)

    circuit._get_pcells()[8]._get_next_unconnected_pin().rename("o1")
    circuit._get_pcells()[9]._get_next_unconnected_pin().rename("o2")
